...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Ongeldig databasevoorvoegsel'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Kan geen database selecteren</h1>
...
<p>Als je niet weet hoe je een database moet configureren neem dan <strong>contact op met je host</strong>. Als niets helpt kun je nog hulp zoeken op de <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress databasefout %1$s bij query %2$s gemaakt door %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'WordPress databasefout %1$s bij query %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Outputtype moet OBJECT, ARRAY_A of ARRAY_N zijn'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
