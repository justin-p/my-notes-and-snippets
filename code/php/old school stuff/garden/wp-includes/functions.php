	...
	foreach ( $wp_tables as $table ) {
		...
		if ( in_array( $table, $tables ) ) {
			...
			if ( defined('WP_REPAIRING') )
				return true;
			// Die with a DB error.
			$wpdb->error = sprintf( /*WP_I18N_NO_TABLES*/'Een of meerdere database tabellen zijn onbeschikbaar. De database moet wellicht worden <a href="%s">gerepareerd</a>.'/*/WP_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed' );
			dead_db();
		}
	}
