...
UPDATE scores SET score=25 WHERE student\_id=4 AND test\_id=4;
...
select where the year of birth\_date is greater than 2000
...
select where the month of birth\_day is 3 OR where the state has the value of WA
...
select all last\_name from students where a value is been supplied. Do not show null data.
...
order the select by last\_name
...
order the select by last\_name but in descending order.
...
CONCAT can be used to 'merge' 2 things into something custom AS can be used to rename something.
...
all students that have name first\_name of 4 unknown letters and a known 5th letter with value of N
...
all students that have name first\_name of 4 or more letters, 4th must have a value of e
